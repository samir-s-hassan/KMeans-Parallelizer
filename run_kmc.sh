...
[ ${#SELECTED_IMPLEMENTATIONS[@]} -eq 0 ] && SELECTED_IMPLEMENTATIONS=("p" "a" "b" "k")
...
for IMPL in "${SELECTED_IMPLEMENTATIONS[@]}"; do
    ...
    if [[ "$IMPL" == "k" ]]; then
        # Ensure KM-CUDA shared library is found at runtime
        cat "$DATASET" | env LD_LIBRARY_PATH="$KMCUDA_LIB:$LD_LIBRARY_PATH" "$EXECUTABLE_PATH" >> "$OUTPUT_FILE" 2>&1
    else
